...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	public Page<Product> findAllInCategory(String categoryIdMatch, 
			Pageable pageable);
...
//			String categoryIdMatch, Integer cityId, String cityIdMatch, Pageable pageable);
	...
	+ "OR p.city.name LIKE %?1% " 
	+ "OR p.category.name LIKE %?1%)")	
public Page<Product> searchInCategoryAndCity(String keyword, 
	String categoryIdMatch, String cityIdMatch, Pageable pageable);
	...
	@Query("SELECT p FROM Product p WHERE (p.category.id = ?1 " 
			...
			+ "OR p.city.name LIKE %?3% " 
			+ "OR p.category.name LIKE %?3%)")
	public Page<Product> searchInCategory(String keyword, String categoryIdMatch,
			Pageable pageable);
...
//	
	...
	@Query("SELECT p FROM Product p WHERE (p.category.id = ?1 "
			+ "OR p.category.allParentIDs LIKE %?2%) AND " 
			+ "p.city.id = ?3 ")	
	public Page<Product> findAllInCategoryAndCity(String categoryIdMatch, 
...
//	
	...
	public Page<Product> findAllInCity(String cityIdMatch, 
			Pageable pageable);
