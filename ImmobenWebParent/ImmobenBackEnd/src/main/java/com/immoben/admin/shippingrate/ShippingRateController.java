...
public class ShippingRateController {
	...
	public String newRate(Model model) {
		...
		model.addAttribute("pageTitle", "Nouveau taux");
	...
	public String saveRate(ShippingRate rate, RedirectAttributes ra) {
		try {
			service.save(rate);
			ra.addFlashAttribute("message", "Le tarif d'expédition a été enregistré avec succès.");
	...
	public String editRate(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			model.addAttribute("pageTitle", "Modifier le taux avec (ID: " + id + ")");
	...
	public String updateCODSupport(@PathVariable(name = "id") Integer id,
		...
		try {
			...
			ra.addFlashAttribute("message", 
					"Le support COD (Mode de payement en espèce) pour l'ID de tarif d'expédition '" + id + "' a été mis à jour.");
		} catch (ShippingRateNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
		}
		return defaultRedirectURL;
	...
	public String deleteRate(@PathVariable(name = "id") Integer id,
		...
		try {
			service.delete(id);
			ra.addFlashAttribute("message", "Le tarif d'expédition avec ID: " + id + " a été supprimé.");
		} catch (ShippingRateNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
		}
		return defaultRedirectURL;
